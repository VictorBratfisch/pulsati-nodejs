...
const lerPassageiro = require('./passageiro').lerPassageiro;
...
http.createServer((req, res) => {
  if (req.method == 'POST') {
    if (req.url.indexOf('/passageiros') >= 0) {
      ...
      lerPassageiro('Nero.json').then((texto) => {
      res.end(texto);
      })
  ...
  http.createServer((req, res) => {
    ...
    if (req.method == 'POST') {
      ...
      lerAeroporto('AeroportoZika.json').then((texto) => {
      res.end(texto);
      })
